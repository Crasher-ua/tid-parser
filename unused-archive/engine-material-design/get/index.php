<?php
	...
	$hostname='localhost';
	$username='crash150_admin';
	$password='x321Ihaveverypowerfulpassword116x10db';
	$database='crash150_db';
...
<?php
	...
	$id = $min_from_db ? $row['min'] : intval(file_get_contents('min.txt'));
	for($i=1;$i<=$POWER_DOWN;$i++)
		$urls[]='https://www.trackitdown.net/track/artist/title/genre/'.($id-$i).'.html';
	$min=$id-$POWER_DOWN;
...
<?php
	...
	//$min=$row['min'];
	//$max=$row['max'];
	echo "<br/>rows: ".$row['rows'].'/'.($max-$min).' ('.$min.'-'.$max.')'.'<br/>';
